"""Tests de la lógica pura del pipeline. No requieren GPU ni FFmpeg."""
...
class FingerprintTest(unittest.TestCase):
    def test_fingerprint_changes_with_heuristics(self):
        first = pc.config_fingerprint(make_config())
        original = dict(pc.SELECTION_HEURISTIC)
        try:
            pc.SELECTION_HEURISTIC["hook_words"] = list(original["hook_words"]) + ["newhook"]
            second = pc.config_fingerprint(make_config())
        finally:
            pc.SELECTION_HEURISTIC.clear()
            pc.SELECTION_HEURISTIC.update(original)
        self.assertNotEqual(first, second)

    def test_fingerprint_changes_with_config(self):
        config = make_config()
        other = pc.PipelineConfig(**{**config.serializable(), "max_clips": 6})
        self.assertNotEqual(pc.config_fingerprint(config), pc.config_fingerprint(other))
...
class ScoringTest(unittest.TestCase):
    def test_hook_words_question_and_ideal_duration(self):
        ...
        self.assertEqual(pc.score_candidate(candidate), 4 + 3 + 8)

    def test_weak_start_penalty(self):
        candidate = {"start": 0.0, "end": 25.0, "text": "Hello everyone, this is important"}
        self.assertEqual(pc.score_candidate(candidate), 4 + 2 - 4)
    ...
    def test_hook_bonus_capped(self):
        ...
        self.assertEqual(pc.score_candidate(candidate), 2 + min(8, 4) * 2)

    def test_overlaps(self):
        first = {"start": 10.0, "end": 20.0}
        second = {"start": 15.0, "end": 25.0}
        disjoint = {"start": 20.0, "end": 30.0}
        self.assertTrue(pc.overlaps(second, [first]))
        self.assertFalse(pc.overlaps(disjoint, [first]))
...
class WordWindowsTest(unittest.TestCase):
    ...
    def test_windows_respect_duration_limits(self):
        windows = pc.word_windows(self.words(10, gap=2.0), min_duration=3.0, max_duration=5.0)
    ...
    def test_windows_can_start_inside_sequence(self):
        windows = pc.word_windows(self.words(6, gap=2.0), min_duration=3.0, max_duration=5.0)
        starts = {window["start"] for window in windows}
        self.assertIn(2.0, starts)
        self.assertIn(4.0, starts)

    def test_empty_words(self):
        self.assertEqual(pc.word_windows([], 18.0, 45.0), [])
...
class BeamSelectTest(unittest.TestCase):
    def test_finds_better_global_than_greedy(self):
        ...
        selected = pc.beam_select(candidates, max_clips=4, beam_width=2)
    ...
    def test_respects_max_clips(self):
        ...
        selected = pc.beam_select(candidates, max_clips=3, beam_width=4)
    ...
    def test_no_overlap_in_result(self):
        ...
        selected = pc.beam_select(candidates, max_clips=2, beam_width=4)
        self.assertFalse(pc.overlaps(selected[0], selected[1:]))
    ...
    def test_negative_scores_fallback_to_best_single(self):
        ...
        selected = pc.beam_select(candidates, max_clips=2, beam_width=2)
...
class SelectClipsTest(unittest.TestCase):
    def test_select_clips_writes_contract(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = pc.select_clips(alignment, output, make_config(min_duration=8.0, max_duration=20.0))
    ...
    def test_select_clips_requires_words(self):
        with tempfile.TemporaryDirectory() as tmp:
            with self.assertRaises(pc.StageValidationError):
                pc.select_clips({"word_segments": []}, Path(tmp) / "selection.json", make_config())
...
class TranscriptQualityTest(unittest.TestCase):
    ...
    def test_ok_quality_metrics(self):
        ...
        quality = pc.transcript_quality(segments, -1.5)
...
class SubtitleGroupingTest(unittest.TestCase):
    ...
    def test_ass_time(self):
        self.assertEqual(pc.ass_time(0.0), "0:00:00.00")
        self.assertEqual(pc.ass_time(3661.25), "1:01:01.25")

    def test_ass_safe_text(self):
        self.assertEqual(pc.ass_safe_text(r"a\b{c}d"), r"a\\b\{c\}d")

    def test_make_ass_content(self):
        words = [self.word(0, 0.5, "hello"), self.word(0.6, 1.0, "world.")]
        content = pc.make_ass_content(words, 518)
...
class VisibilityTest(unittest.TestCase):
    def test_pixel_diff_ratio(self):
        ...
        self.assertEqual(pc.pixel_diff_ratio(base, base), 0.0)
        altered = bytearray(base)
        altered[9] = 255
        self.assertEqual(pc.pixel_diff_ratio(base, bytes(altered)), 0.25)

    def test_pixel_diff_ratio_mismatched_sizes(self):
        with self.assertRaises(pc.StageValidationError):
            pc.pixel_diff_ratio(b"\x00" * 6, b"\x00" * 9)

    def test_required_threshold(self):
        self.assertEqual(pc.required_visibility_threshold(0.0), 0.005)
        self.assertEqual(pc.required_visibility_threshold(0.01), 0.032)
...
class PruneWorkTest(unittest.TestCase):
    def test_prunes_only_stale_runs(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            removed = pc.prune_work(root, max_age_days=10)
    ...
    def test_disabled_by_default(self):
        with tempfile.TemporaryDirectory() as tmp:
            self.assertEqual(pc.prune_work(Path(tmp), max_age_days=0), 0)
...
class ValidateVideoTest(unittest.TestCase):
    def test_missing_duration_is_controlled_error(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(pc, "ffprobe_json", return_value=probe):
                with self.assertRaises(pc.StageValidationError):
                    pc.validate_video(video, expected_duration=10.0, ffprobe="ffprobe", logger=None)
